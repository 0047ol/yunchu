...
var click = 'click';
//举报提交
$('.dialog-report-send-btn').on(click,function(event) {
    ...
    //if (checkCaptcha_code(report_captcha_code,captcha_code_token)) {
		$.ajax({
			...
			data:  'id=' + report_id + '&' + $('.dialog-report-form').serialize() + '&token=' + user_token + '&type=' + report_type + '&from=' + report_from + '&types=' + getReportProductTypes(report_url) + '&code=' + report_captcha_code + '&codetoken=' + captcha_code_token,
...
//
$(document).on(click,'.dialog-report-list-item',function(event) {
...
//
$('.dialog-report-content-input').on(click,function(event) {
...
//点击注册
$('.toolbar-register-btn').on(click,function(event) {
	dialog_register.open();
});
//点击登录
$('.toolbar-login-btn').on(click,function(event) {
	dialog_login.open();
});
//回到个人主页
$('.toolbar-user-btn').on(click,function(event) {
	locaUrl('./home/#' + getHash(),false);
});
//打开注册对话框完毕
$('.mc-register').on('opened.mdui.dialog',function(event) {
...
//开始打开登录对话框
$('.mc-login').on('opened.mdui.dialog',function(event) {
	if (isPC()) {
	refreshQRcode();
	}
});
document.querySelector('.mc-register').addEventListener('open.mdui.dialog', function () {
  sendYunChuAPP("openRegistDlg",0);
  //window.location.hash = 'dialog';
});
document.querySelector('.mc-register').addEventListener('close.mdui.dialog', function () {
  sendYunChuAPP("closeRegistDlg",0);
  //window.location.hash = '';
});
document.querySelector('.mc-report').addEventListener('opened.mdui.dialog', function () {
  setCaptcha_code();
	showChild('.dialog-report-captcha-code');
});
document.querySelector('.mc-login').addEventListener('open.mdui.dialog', function () {
  sendYunChuAPP("openLoginDlg",0);
  //window.location.hash = 'dialog';
});
document.querySelector('.mc-login').addEventListener('close.mdui.dialog', function () {
...
$('.dialog-login-app-login-btn').on(click,function(event) {
if (isMQQ()) {
...
//注册对话框准备
$('.dialog-register-step-one-next-step-btn').on(click,function(event) {
...
//获取注册邀请码
$('.dialog-register-step-one-get-invitation-code-btn').on(click,function(event) {
...
//注册对话框用户协议
$('.dialog-register-step-one-privacy-agreement-btn').on(click,function(event) {
if (!isYunChuApp()) {
	hideChild('.dialog-register-step-one');
...
//读完注册用户协议
$('.dialog-register-privacy-agreement-submit-btn').on(click,function(event) {
...
function checkCaptcha_code(input_code) {
	...
	$.ajax({
		method: 'POST',
		async : false,
		url: 'https://wd.cn.ecsxs.com/api/v1/user/imagecode/?token=' + captcha_code_token + '&code=' + input_code,
		success: function(data) {
			var json = JSON.parse(data);
			var json_state = json['state'];
			if (json_state == 200) {
...
//用户协议勾选事件
$('.dialog-register-step-one-privacy-agreement-checkbox').on('change',function(event) {
...
//注册准备 验证
var register_token;
$('.dialog-register-step-one-send-email-btn').on(click,function(event) {
  var register_invitation_code = getValue('.dialog-register-step-one-invitation-code-input');
  var register_email = getValue('.dialog-register-step-one-email-input');
  var register_privacy_agreement_checkbox = getChecked('.dialog-register-step-one-privacy-agreement-checkbox');
  var register_captcha_code = getValue('.dialog-register-step-one-captcha-code-input');
  if (!register_privacy_agreement_checkbox) {
    ...
    $.ajax({
      ...
      data: 'invitationcode=' + register_invitation_code + '&email=' + register_email,
      success: function(data) {
        var json = JSON.parse(data);
		var json_state = json['state'];
        if (json_state == 200) {
...
//完善注册 提交
$('.dialog-register-step-two-submit-btn').on(click,function(event) {
  var register_invitation_code = getValue('.dialog-register-step-one-invitation-code-input');
  var register_email = getValue('.dialog-register-step-one-email-input');
  var register_email_code = getValue('.dialog-register-step-one-email-code-input');
  ...
  if (isEmpty(register_user_name)) {
    ...
    $.ajax({
      ...
      data: 'username=' + register_user_name + '&password=' + register_password_two + '&invitationcode=' + register_invitation_code + '&email=' + register_email + '&code=' + register_email_code + '&mobile=' + register_user_phone + '&qq=' + register_user_qq,
      success: function(data) {
        var json = JSON.parse(data);
		var json_state = json['state'];
        if (json_state == 200) {
...
//扫码登录返回账号登录
$('.dialog-login-scanQRcode-return-login-btn').on(click,function(event) {
	dialog_return_login(0);
});
//账号登录返回扫码登录
$('.dialog-login-return-login-scanQRcode-btn').on(click,function(event) {
	hideChild('.dialog-login');
...
//账号登录返回App登录
$('.dialog-login-applogin-btn').on(click,function(event) {
	hideChild('.dialog-login');
...
//登录对话框
var failure = 0;
$('.dialog-login-btn').on(click,function(event) {
  ...
  if (isEmpty(login_username)) {
	...
	if (failure >= 1 && !isEmpty(login_captcha_code) && !checkCaptcha_code(login_captcha_code)) {
      ...
      data: login_method + '&password=' + login_password + '&date=' + login_date,
	  headers: yc_app_ajax_header,
	  crossDomain:true,
      success: function(data) {
        var json = JSON.parse(data);
...
//刷新登录二维码
$('.dialog-login-scanQRcode-step-refresh-btn').on(click,function(event) {
	refreshQRcode();
});
//重置密码对话框
$('.dialog-reset-password-btn').on(click,function(event) {
	hideChild('.dialog-login');
	hideChild('.dialog-login-scanQRcode');
	hideChild('.dialog-login-applogin');
	setStyle('.mc-login','top:63.4px;height:603.2px');
...
//重置密码输入邮箱
$('.dialog-reset-password-email-input').on('keyup',function(event) {
	if (isEmails(event['srcElement'].value)) {
		showChild('.dialog-reset-password-captcha-code');
	}
});
//找回用户名
$('.dialog-reset-password-retrieve-email-btn').on(click,function(event) {
  ...
  if (isEmpty(reset_password_email)) {
    ...
    $.ajax({
      method: 'POST',
      url: 'https://wd.cn.ecsxs.com/api/v1/user/code/',
      data: 'type=retrieve&email=' + reset_password_email,
      success: function(data) {
        var json = JSON.parse(data);
		var json_state = json['state'];
        if (json_state == 200) {
...
//重置密码 验证
$('.dialog-reset-password-send-email-btn').on(click,function(event) {
  var reset_password_email = getValue('.dialog-reset-password-email-input');
  ...
  if (isEmpty(reset_password_email)) {
    ...
    $.ajax({
      method: 'POST',
      url: 'https://wd.cn.ecsxs.com/api/v1/user/code/',
      data: 'type=reset&username=' + reset_password_username + '&email=' + reset_password_email,
      success: function(data) {
        var json = JSON.parse(data);
		var json_state = json['state'];
        if (json_state == 200) {
...
//重置密码 提交
$('.dialog-reset-password-submit-btn').on(click,function(event) {
  var reset_password_email = getValue('.dialog-reset-password-email-input');
  var reset_password_username = getValue('.dialog-reset-password-username-input');
  ...
  if (isEmpty(reset_password_email)) {
    ...
    $.ajax({
      ...
      data: 'username=' + reset_password_username + '&email=' + reset_password_email + '&code=' + reset_password_email_code,
      success: function(data) {
        var json = JSON.parse(data);
		var json_state = json['state'];
        if (json_state == 200) {
...
//重置密码返回登录对话框
$('.dialog-reset-password-return-login-btn').on(click,function(event) {
	if (isYunChuApp()) {
