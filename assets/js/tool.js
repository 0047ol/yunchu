...
//开关侧滑抽屉
$('.mdui-drawer-opens').on('click',function(e) {
...
//抽屉项目点击
$('.mdui-drawer-closes').on('click',function(e) {
	if (!isPC()) {
		mdui_drawer.close();
	}
});
...
//默认返回给云储App的点击关闭事件
$(document).on('click', '.yunchu-js-interfaces-back-btn', function (event) {
	if (!isYunChuApp()) {
		if (getParam('from') == 'home') {
		locaUrl('/home/#' + getHash(),false);
		} else {
...
//通用复制提示
$('.clipboard-copy-toast').on('click',function(event) {
	showToast('已复制');
});
//Toast
...
function removeCookie(cookie_name) {
	document.cookie = cookie_name + '=0; expires=Thu,01 Jan 1970 00:00:00 UTC; path=/; domain=cxoip.com; ';
};
//读取SESSION
...
function addStrMosaic(str) {
	if (null != str && str != undefined && str != '未绑定') {
		if (isPhoneNum(str) || isQQNum(str)) {
			return str.substring(0,3) + "*****" + str.substr(-4,4);
        } else {
			if (str.length <= 3) {
...
function isHTML(str) {
   var reg = /<[a-z][\s\S]*>/i;
   if ((validationStart(str,'<html') || validationStart(str,'<!DOCTYPE')) && validationEnd(str,'</html>')) {
	   return true;
   }
   return false;
};
